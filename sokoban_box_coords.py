...
class SokobanEnvironment(gym.Env):
    def __init__(self, load_file: str = 'Levels/level_0.txt', max_steps: int = 100):
        ...
        # self.observation_space = gym.spaces.Discrete(self.max_states)  # one state for each tile of maze
...
class SemiGradientSarsa:
    ...
    def x_s_a(self, state, action):
        """
        finds the X vector for a specific state -> incorporates all box locations and agent location
        """
        boxes = [[i for i in box] for box in state['boxes']]  # create list of individual box coordinated
        return np.append(np.array([1, state['agent'][0], state['agent'][1]]), boxes)  # feature vector
    ...
    def train(self, episodes):
        ...
        """
        weights = np.zeros(len(self.env._box_loc) * 2 + len(self.env._agent_loc) + 1)  # dynamically create weight vector based on number of boxes
        total_rewards = []
        print_count = 0
        for i in range(episodes):
            ...
            while not done:
                ...
                if done:
                    ...
                    weights += self.alpha * (reward + 0.9 * self.q_hat(state_prime, action_prime, weights) - self.q_hat(state, action, weights)) * self.x_s_a(state, action)
                    state = state_prime
                    action = action_prime
...
if __name__ == '__main__':
    ...
    if SAVE_FIG:
        plt.savefig(f"pres_coords_{a}_e{e}_{episodes}.svg")  # save figure
    plt.show()
